...
def cons_ws_2(params):
    return params[2] - (params[0] + 10)  
def cons_ws_3(params):
    return params[2] - (params[1] + 10)
def cons_cp_1(params):
    return params[0] - (params[1] + 20)  
def cons_cp_2(params):
    return params[0] - (params[2] + 20)  
def cons_cp_3(params):
    return params[1] - (params[2] + 20)
...
def alt_pay_vs_damage(damages, optimized_step1, optimized_step2, optimized_step3, haz_int, nominal, include_plot=False):
    ...
    if include_plot:
        ...
        damage_flt[damage_flt == 0] = 1e-6  # Replace 0 with a small value for plotting
        pay_dam_df_copy = pay_dam_df.copy()
        pay_dam_df_copy['damage'][pay_dam_df_copy['damage'] == 0] = 1e-6  # Replace 0 with a small value for plotting
        ...
        ax1.set_title("Damage vs. Payout - Damage <= Nominal - Log")
        ax1.set_xlabel("Damage [USD]")
        ax1.set_ylabel("Payout [USD]")
        ax1.legend(loc='upper left', borderpad=2.0)

        ax2.scatter(pay_dam_df_copy['damage'], pay_dam_df_copy['pay'], marker='o', color='blue', label='Events')
